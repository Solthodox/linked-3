...
contract JobSearch is userBase, CompanyBase, Referrals {
    ...
    */
        ...
        payLinked3(_useReferral(referralCode, newUserPrice()))
    {
        require(
            ...
            "Sorry, only EOAs can call this function"
        );
    ...
    */
    function registerCompany(
        string memory name,
        string memory mainPageURI,
        bytes memory referralCode
    ) public payLinked3(_useReferral(referralCode, newCompanyPrice())) {
        require(
            !_isContract(msg.sender),
            "Sorry, only EOAs can call this function"
        );
    ...
    * Companies must pay for posting the offer
    ...
    function newJobOffer(
        ...
        uint256 experience,
        int256 trustLevel
    ) public onlyCompanies payLinked3(newJobOfferPrice()) {
    ...
    * @dev It reverts if : 
    ...
    function applyForJob(uint256 offerId) public onlyUsers {
        ...
        require(
            _offers[offerId].experience <=
                users()[addressToUser(msg.sender)].experience &&
                _offers[offerId].trustLevel <=
                users()[addressToUser(msg.sender)].trustLevel,
    ...
    * @dev It adds the developer to the company's team and also the salary to the devloper's profile
    ...
    function hire(
        ...
        uint256 _collateral
    ) public payable onlyCompanies {
        require(msg.value >= _collateral, "Colateral not deposited");
        require(
            users()[addressToUser(developer)].account != address(0),
            "Developer not found"
        );
        ...
        address(newSalaryContract).call{value: msg.value}("");
    ...
    function _salaryVerified(address contractAddress)
        internal
        view
        returns (bool)
    {
